...
abstract class Parametre( // cette classe permet de créer un modèle pour afficher les cours de l'emploi du temps, elle est abstraite car elle ne peut pas être instanciée
    ...
    protected val popupContent: ConstraintLayout
) {
    ...
    protected fun changePopupVisibility(visible: Boolean) {
        if (!visible) {
        ...
        (popupContent.parent as View).visibility = if (visible) View.VISIBLE else View.GONE
    }
}
