...
abstract class Parametre( // cette classe permet de créer un modèle pour afficher les paramètres, elle est abstraite car elle ne peut pas être instanciée
    protected val session: SessionEntity? = null, // la session utilisée par le view model des paramètres
    protected val popupContent: ConstraintLayout? = null, // le layout du popup où certains outils peuvent être utilisés
    protected val onConfirmLiveData: MutableLiveData<() -> Unit>? = null // un live data qui permet de changer la fonction de callback du bouton de confirmation
) {
    ...
    protected fun changePopupVisibility(visible: Boolean) { // cette fonction permet de changer la visibilité du popup
        if (popupContent == null) return // si le popup est null on ne fait rien
