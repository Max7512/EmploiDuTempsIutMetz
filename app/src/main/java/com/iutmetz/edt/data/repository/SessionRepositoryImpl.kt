...
class SessionRepositoryImpl(
    ...
    override suspend fun getSession(): SessionEntity? { // cette fonction renvoie la session sauvegardée dans la base de données locale ou null si aucune session n'est sauvegardée
        val session =
            sessionDao.getSession() // une liste de session est renvoyée par la base de données locale
        return if (session.isNotEmpty()) {
            session.first()
        } else {
            null
        } // si la liste n'est pas vide on renvoie la première session sinon on renvoie null
    }
    ...
    override suspend fun saveSession(session: SessionEntity) { // cette fonction sauvegarde la session dans la base de données locale
        val savedSession =
            getSession() // on récupère la session sauvegardée dans la base de données locale
        if (savedSession == null) {
            ...
            sessionDao.update(
                ...
                session.groupe,
                session.coursColor,
                session.coursTextColor,
                session.bandeauColor,
                session.bandeauTextColor,
                session.saeColor,
                session.saeTextColor,
                session.todayBackgroundColor
            ) // sinon on met à jour la session avec les nouvelles valeurs
        }
    }

    override suspend fun checkVersion(githubBaseUrl: String): String? { // cette fonction retourne la dernière version disponible sur le projet github ou null si une erreur se produit
        val result = NetworkResponse.getResponse(
            retrofit,
            { apiService.getTags("$githubBaseUrl/tags") },
            ""
        ) // on récupère les tags du projet github
